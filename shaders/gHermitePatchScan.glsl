...
// emits a trinagle (v0, v1, v2) with normal
// where vi is in clip coordinates
void emitTriangle(vec4 v0, vec4 v1, vec4 v2, vec3 normal) {
    gl_Position = v0;
    n = normal;
    EmitVertex();
    gl_Position = v1;
...
void emitPatch(Position v0, Position v1, Position v2, Position v3) {
    vec3 normal = normalize(cross(v1.world - v0.world, v2.world - v0.world));
    emitTriangle(v0.clip, v1.clip, v2.clip, normal);

    normal = normalize(cross(v3.world - v1.world, v2.world - v1.world));
...
void main() {
    ...
    // emit input triangle
    emitTriangle(v_in[0].pos.clip, v_in[1].pos.clip, v_in[2].pos.clip, normal);
}
