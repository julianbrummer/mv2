...
bool VertexNode::operator <(const VertexNode& other) const {
    return surfaceIndex < other.surfaceIndex;
}
...
void DMCVerticesBuilder::handle(const DMCOctreeCell *node, const Index &index) {
    ...
    for (const shared_ptr<VertexNode> vNode : node->vertices)
        positions.push_back(2*cellGridRadius*vNode->v+origin);
...
void DMCCellBuilder::handle(const DMCOctreeCell *node, const Index &index) {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        Vector3f vCorner = (2*cellGridRadius/pow2(node->level))*Vector3f(corner.x, corner.y, corner.z);
        corners[i] = origin + vCorner;
    }
    // push cell edges
    for (int i = 0; i < 12; ++i) {
...
void DualMarchingCubes::clusterCell(const Index& cell_index, DMCOctreeCell* node) {
    if (node->hasChildren()) {
        ...
        sort(vertices.begin(), vertices.end()); // by surface index
...
void DualMarchingCubes::bfs(IMeshBuilder& builder, bool processEmptyNodes) const {
...
void DualMarchingCubes::vertices(float voxelGridRadius, aligned_vector3f &positions,
                                 vector_4_uint& offset, vector_4_uint& count) const {
    float cellGridRadius = voxelGridRadius-voxelGridRadius/sampler->size;
    float o = -cellGridRadius;
    Vector3f origin(o,o,o);
    initVector(offset);
    initVector(count);
    DMCVerticesBuilder builder(cellGridRadius, origin, positions, offset, count);
    bfs(builder, true);
}

void DualMarchingCubes::cells(float voxelGridRadius, aligned_vector3f &positions,
    ...
    Vector3f origin(o,o,o);
    initVector(offset);
    DMCCellBuilder builder(sampler->res, cellGridRadius, origin, positions, offset);
    bfs(builder, true);
}
